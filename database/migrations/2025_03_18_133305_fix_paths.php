...
return new class extends Migration
{
    ...
    {
        $arquivos = Arquivo::where(function ($query) {
            $query->whereNotNull('atividade_id')
                  ->orWhereNotNull('submissao_atividade_id')
                  ->orWhereNotNull('modelo_documento_id')
                  ->orWhereNotNull('orientacao_id');
        })
        ->with([
        ...
        DB::transaction(function() use($arquivos){
            foreach($arquivos as $arquivo) {
                ...
                if(!empty($arquivo->atividade_id)){
                ...
                } else if(!empty($arquivo->submissao_atividade_id)){
                    ...
                    Storage::disk('public')->move($arquivo->caminhoEnome(), $novoCaminhoEnome);
                } else if(!empty($arquivo->orientacao_id)) {
