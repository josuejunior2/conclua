            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="sidebar-menu">
                    <ul class="navbar-nav pt-lg-3">
                        @if (isset($semestres))
                            <div class="mb-3">
                                <form id="form_mudar_semestre" method="post"
                                    action="{{ route('semestre.mudar-semestre') }}">
                                    @csrf
                                    <!-- <button type="submit">Excluir</button>  -->
                                    <select class="form-select" name="semestre_id" id="semestre_id">
                                        @foreach ($semestres as $semestre)
                                            <option value="{{ $semestre->id }}"
                                                {{ $semestre->id == session('semestre_id') ? 'selected' : '' }}>
                        ...
                        <li class="nav-item">
                            <a class="nav-link"
                                href="{{ route('academico.show', ['user' => auth()->guard('web')->user()]) }}">
                                <span
            ...
            $(document).ready(function() {
                ...
                semestres.forEach(function(index) {
                    if (index.id == $('#semestre_id').val()) {
                        ...
                        $('.inicio').html(
                            `Início: ${data_inicio.getDate().toString().padStart(2, '0')}/${(data_inicio.getMonth() + 1).toString().padStart(2, '0')}`
                            );
                        $('.fim').html(
                            `Fim: ${data_fim.getDate().toString().padStart(2, '0')}/${(data_fim.getMonth() + 1).toString().padStart(2, '0')}`
                            );
                    }
                });
            });
